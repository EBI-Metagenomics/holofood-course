...
(note later versions of BEAGLE do not accept ANGSD `.beagle` files as input). 
...
The other columns correspond to the genotype probabilities for each individual, in sets of three: major/major genotype probability, 
major/minor genotype probability and minor/minor genotype probability.
...
GWAS can also be affected by population genetic structure (see [Price et al. 2006](https://www.nature.com/articles/ng1847)), which should also be taken into account in the model. Previously population structure was evaluated for this dataset using PCangsd, to produce a PCA. From this PCA, we can see that the salmon population is highly related, but some population structure is evident:
